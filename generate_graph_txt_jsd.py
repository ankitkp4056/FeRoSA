...
print '.....Loading cos-sim-array from jsd_mat.npy.....'
cos_sim_array = np.load("lda_5k/jsd/jsd_mat.npy")
...
def cos_sim(p1, p2):
    return 1-cos_sim_array[paper_array.index(p1), paper_array.index(p2)]
...
def cos_sim_top(p1):
    ...
    for i in xrange(len(similarity_p1)):
        similarity_p1[i] = 1-similarity_p1[i]
...
def get_graph_txt(paper_id, out_net, in_net,facet=None):
    ...
    if facet:
        name = "Walker/graph/jsd/" + paper_id +facet + '_graph_jsd.txt'
    else:
        name = "Walker/graph/jsd/" + paper_id + '_graph_jsd.txt'
    ...
    file = open('Walker/graph/jsd/'+paper_id+'_seed.txt', 'w')
    file.write(paper_id)
    file.close()
